		...
		<div className="space-y-8">
			...
			{totalTime > 0 && (
				<div className="bg-amber-50 dark:bg-amber-950/50 border border-amber-200 dark:border-amber-800 rounded-xl p-5 flex items-center gap-3">
					...
					<span className="text-base text-amber-900 dark:text-amber-100">
						<TextAnimate animation="fadeIn" delay={0.1} by="word" once>
			...
			<div className="space-y-10">
				{Object.entries(groupedSteps).map(
					([section, sectionSteps], sectionIndex) => (
						<div key={section} className="space-y-5">
							...
							<div className="space-y-5">
								{sectionSteps.map((step, index) => {
									...
									// Find the next uncompleted step globally across all sections
									const nextUncompletedStep = steps.find(
										(s) => !completedSteps.has(s.stepNumber)
									)
									const isNext =
										nextUncompletedStep?.stepNumber === step.stepNumber
										...
										>
											<div className="flex gap-6">
												<div className="flex-shrink-0 flex flex-col items-center gap-4">
													<div
														className={`flex size-14 items-center justify-center rounded-2xl font-bold text-xl transition-all ${
															isCompleted
													...
													<Checkbox
														...
														checked={isCompleted}
														onCheckedChange={() =>
															handleStepToggle(step.stepNumber)
														}
														className="size-4 md:size-5 data-[state=checked]:bg-green-500 data-[state=checked]:border-green-500"
													/>
												</div>
												...
												<div className="flex-1 space-y-4">
													<div className="flex items-start justify-between gap-4">
														...
														{isNext && !isCompleted && (
															<Badge
																variant="default"
																className="hidden md:flex bg-blue-500 hover:bg-blue-600 shrink-0 px-3 py-1"
															>
